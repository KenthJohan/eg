...
{
	EgOdrivesCmd const *c = ecs_field(it, EgOdrivesCmd, 1); // input
	EgCanSignal *s = ecs_field(it, EgCanSignal, 2);         // output
...
void EgOdrivesImport(ecs_world_t *world)
{
	...
	*/
	...
	.query.filter.terms =
	{
	{.id = ecs_id(EgOdrivesCmd), .inout = EcsIn},
	{.id = ecs_id(EgCanSignal), .inout = EcsOut},
	}});
}