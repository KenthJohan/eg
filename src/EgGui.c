...
void System_Hover1(ecs_iter_t* it)
{
	...
	const EgPositionGlobal_V2F32 *p   = ecs_field(it, EgPositionGlobal_V2F32, 2); // [in] GUI Element Position
	const EgRectangle_V2F32      *r   = ecs_field(it, EgRectangle_V2F32,      3); // [in] GUI Element Rectangle
	const EgPosition_V2F32       *mp0 = ecs_field(it, EgPosition_V2F32,       4); // [in] Userinput Mouse Position
	      EgHover                *h0  = ecs_field(it, EgHover,                5); // [inout] Hover entity 
	      EgPosition_V2F32       *hp0 = ecs_field(it, EgPosition_V2F32,       6); // [out] Hover relative mouse position 
    ...
    for (int i = 0; i < it->count; i ++)
	{
		...
		hp0->x = mp0->x - p[i].x;
		hp0->y = mp0->y - p[i].y;
		int hit = ((hp0->x > 0) && (hp0->x < r[i].w)) && ((hp0->y > 0) && (hp0->y < r[i].h));
		if(hit)
		{
			//printf("Hover: %i\n", it->entities[i]);
...
{
	const EgPosition_V2F32 *mp0  = ecs_field(it, EgPosition_V2F32,   1); // [in] Userinput Mouse Position
	const EgMouse          *ms0  = ecs_field(it, EgMouse,            2); // [in] Userinput Mouse State
	      EgHover          *h0   = ecs_field(it, EgHover,            3); // [inout] Hover entity
	const EgPosition_V2F32 *hp0  = ecs_field(it, EgPosition_V2F32,   4); // [in] Hover position relative
	      EgGuiDrag        *drag = ecs_field(it, EgGuiDrag,          5); //
	      EgPosition_V2F32 *dp0  = ecs_field(it, EgPosition_V2F32,   6); //
...
void System_Margin(ecs_iter_t *it)
{
    ...
    for (int i = 0; i < it->count; i ++)
	{
		p[i].x = m->left;
		p[i].y = m->top;
		r[i].w = r0[0].w - (m->left + m->right);
		r[i].h = r0[0].h - (m->botton + m->top);
	}
}
