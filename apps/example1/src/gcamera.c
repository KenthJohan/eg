...
static void quaternion_rotation_procedure(float const look[3], float q[4], float speed)
{
	...
	qf32_xyza(dq_pitch, 1.0f, 0.0f, 0.0f, look[0] * speed); // Make delta pitch quaternion
	qf32_xyza(dq_yaw, 0.0f, 1.0f, 0.0f, look[1] * speed);   // Make delta yaw quaternion
	qf32_xyza(dq_roll, 0.0f, 0.0f, 1.0f, look[2] * speed);  // Make delta roll quaternion
...
void gcamera_update(gcamera_state_t *camera, float delta, float w, float h)
{
	...
	//printf("gcamera:\n");
	//v4f32_print(camera->q);
	//m4f32_print(&camera->vp);
