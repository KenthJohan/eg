...
void frame(void) {
    ...
    if (state.show_another_window) {
        ...
        {
            igTableSetupColumn("A0", 0, 0, 0);
            igTableSetupColumn("A1", 0, 0, 0);
            igTableHeadersRow();
            igTableNextColumn();
            igText("A0 Row 0");
            {
				float tbh = igGetTextLineHeightWithSpacing();
                float rows_height = tbh * 2;
                if (igBeginTable("table_nested2", 2, ImGuiTableFlags_Borders | ImGuiTableFlags_Resizable | ImGuiTableFlags_Reorderable | ImGuiTableFlags_Hideable, (ImVec2){0,0}, 0))
                {
                    igTableSetupColumn("B0", 0, 0, 0);
                    igTableSetupColumn("B1", 0, 0, 0);
                    igTableHeadersRow();
                    igTableNextRow(ImGuiTableRowFlags_None, rows_height);
                    igTableNextColumn();
                    igText("B0 Row 0");
                    igTableNextColumn();
                    igText("B1 Row 0");
            ...
            igTableNextColumn(); igText("A1 Row 0");
            igTableNextColumn(); igText("A0 Row 1");
            igTableNextColumn(); igText("A1 Row 1");
            igEndTable();
        }
