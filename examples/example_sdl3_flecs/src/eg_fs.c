...
/* you can use '/' for a path separator on Windows, but to make the log output look correct, we'll #ifdef this... */
	...
	if (!SDL_GetPathInfo(fullpath, &info)) {
		...
		//SDL_Log("%s (type=%s, size=%" SDL_PRIu64 ", create=%" SDL_PRIu64 ", mod=%" SDL_PRIu64 ", access=%" SDL_PRIu64 ") %jx",fullpath, type, info.size, info.modify_time, info.create_time, info.access_time, e);
